...
static bool upipe_ts_emmd_table_validate(struct upipe *upipe)
{
    ...
    upipe_ts_psid_table_foreach (upipe_ts_emmd->next_emm, section_uref) {
        ...
        if (!emm_validate(section) || !psi_check_crc(section)) {
...
static int decrypt(struct upipe *upipe, uint8_t *esd, size_t n)
{
    ...
    if (err) {
        upipe_err_va(upipe, "decrypt failed: %s", gcry_strerror(err));
        return ret;
    }
                                ...
                                struct upump **upump_p)
{
    ...
    upipe_ts_psid_table_foreach (upipe_ts_emmd->next_emm, section_uref) {
        ...
        uint8_t last_table_id = emm_get_last_table_id(section); // XXX ?
        uint8_t cipher = emm_get_emm_cipher_type(section);
        if (cipher != EMM_CIPHER_RSA_2048_OAEP) {
            // XXX ?
        }

        upipe_ts_emmd_parse_descs(upipe, flow_def,
                emm_get_descl_const(section), emm_get_desclength(section));

        int j = 0;
        uint8_t *emm_n;
        while ((emm_n = emm_get_emmn((uint8_t *)section, j++)) != NULL) {
            uint8_t ekid[8];
            emmn_get_ekid(emm_n, ekid);
 ...
 * @return an error code
 */
static void upipe_ts_emmd_ecm_parse_sd_descs(struct upipe *upipe,
                                      struct uref *flow_def,
                                      const uint8_t *descl, uint16_t desclength)
{
    const uint8_t *desc;
